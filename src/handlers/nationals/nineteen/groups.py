...
SCHEDULING_BASE_PATH='https://usnationals2019.appspot.com'
...
class Groups2019Handler(BaseHandler):
  def get(self, person_id='', event_id='', round_num='', stage='', group=''):
    ...
    template = JINJA_ENVIRONMENT.get_template('nationals/2019/groups.html')
    ...
    if not person_id and self.user and self.user.wca_person and self.user.wca_person.id() in competitors_by_id and not event_id:
      self.redirect_to('groups_2019_person', person_id=self.user.wca_person.id())
      return
